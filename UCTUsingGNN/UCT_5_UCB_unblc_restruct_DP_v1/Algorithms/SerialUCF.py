...
def get_multi_k_sim_results(sim, uct_simulators, configs, total_query, anal_results, simu_results):
    effis = []
    max_result = 0
    ...
    if configs['reward_method'] == 'analytics':
        ...
        for k in configs['topk_list']:
            ...
            effis = [sim.get_effi_info_using_anal()]
            if len(sim.topk) == 0:
                max_sim_reward_result = {'max_sim_state': 'empty', 'max_sim_reward': 0, 'max_sim_para': -1,
                                         'max_sim_effi': -1, 'max_sim_vout': -500}
            else:
            ...
            if effis != []:
                anal_result = [effis[0]['efficiency'], effis[0]['output_voltage'], max_result,
                               str(sim.current.parameters), total_query]
        ...
        for k in configs['topk_list']:
            if effis != []:
                anal_results[k].append([effis[0]['efficiency'], effis[0]['output_voltage'], max_result,
                                        str(sim.current.parameters), total_query])
            simu_results[k].append(
                [max_sim_reward_result['max_sim_effi'], max_sim_reward_result['max_sim_vout'],
                 max_sim_reward_result['max_sim_reward'],
...
def serial_UCF_test(trajectory, test_number, configs, result_folder, Sim=None, uct_tree_list=None, keep_uct_tree=False):
    ...
    anal_results = {}
    for k in configs['topk_list']:
        simu_results[k] = [['efficiency', 'vout', 'reward', 'DC_para', 'query']]
        anal_results[k] = [['efficiency', 'vout', 'reward', 'DC_para', 'query']]
    ...
    for test_idx in range(test_number):
        ...
        effis = []
        if inside_sim:
            anal_results, simu_results, effis = get_multi_k_sim_results(sim, uct_simulators, configs, total_query,
