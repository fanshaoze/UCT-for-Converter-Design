...
def serial_UCF_test(depth_list, trajectory, configs, date_str, Sim=None, uct_tree_list=None, keep_uct_tree=False):
        ...
        for num_runs in trajectory:
            ...
            for j in range(0, init_nums):
                ...
                for i in range(0, int(configs["game_num"] / init_nums)):
                    ...
                    sim = Sim(sim_configs, configs['num_component'])
                    ...
                    for n in range(configs["tree_num"]):
                        uct_simulators.append(Sim(sim_configs, configs['num_component']))
                        ...
                        if initialize_tree:
                            ...
                            # when keeping the tree, we need to assign new simulators to them (the reward function may change)
                    ...
                    while not sim.is_terminal():
                        ...
                        for n in range(configs["tree_num"]):
                            ...
                            tree_size_tmp, tree_tmp, depth = uct_tree_list[n].plan(False, sim.current.step-(len(edges)+len(init_nodes)))
                        ...
                        if sim.get_state().graph_is_valid():
                            visited_states.append(sim.get_state())
                        final_reward = r
                    ...
                    # Shun: this evaluation is not needed
                    effis = None
    ...
    fo.close()
    # hacking here to return necessary info
    return {'sim': sim,
