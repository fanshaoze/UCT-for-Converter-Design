...
def merge_leaf_info_into_main_sim(uct_simulator, leaf_sims):
    """
    ...
    for uct_simu in leaf_sims:
        if uct_simu.current_max['reward'] > uct_simulator.current_max['reward']:
            uct_simulator.current_max = uct_simu.current_max
        uct_simulator.new_query_time += uct_simu.new_query_time
        uct_simulator.new_query_counter += uct_simu.new_query_counter
        # self.topk.append([self.current.duplicate(), self.reward, self.current.parameters, key_info])
        for top in uct_simu.topk:
            if not uct_simulator.topk_include_current(top[3]):
                uct_simulator.topk.append(top)
    uct_simulator.topk.sort(key=keyFunc)
    for top in uct_simulator.topk:
        print(top[1], ' ', top[3])
...
def parallel_get_multi_k_sim_results(sim, configs, total_query, start_time, total_planning_time,
    ...
    shared_key_sim_effi_ = parallel_topk_simulation(thread_num=configs['leaf_thread_num'], topks=sim.topk,
...
def leaf_parallel_UCF_test(trajectory, test_number, configs, result_folder, Sim=None, uct_tree_list=None,
                           keep_uct_tree=False):
    ...
    for test_idx in range(test_number):
        ...
        # init the leaf simulators

        isom_topo_dict = {}
        reward_hash = Manager().dict()
        ...
        for n in range(configs["tree_num"]):
            uct_simulators.append(Sim(sim_configs, approved_path_freq,
                                      ...
                                      None, configs['num_component'], filter_sim))
            _leaf_sims = []
            for _ in range(configs["leaf_thread_num"]):
                _leaf_sims.append(Sim(sim_configs, approved_path_freq,
                                      component_condition_prob,
                                      key_expression, _unblc_comp_set_mapping, component_priorities,
                                      key_sim_effi_for_anal,
                                      None, configs['num_component'], filter_sim))
            # init the shared reward hash
            ...
            uct_tree_list.append(
                uct.UCTPlanner(uct_simulators[n], -1, int(num_runs / configs["tree_num"]),
                               ...
                               configs['path_default_policy'], _leaf_sims, reward_hash))
        ...
        for tmp_uct_tree in uct_tree_list:
            ...
            merge_leaf_info_into_main_sim(tmp_uct_tree.sim_, tmp_uct_tree.leaf_sims)
