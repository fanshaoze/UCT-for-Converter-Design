...
from Algorithms.LeafParallelUCF import leaf_parallel_UCF_test
...
def main(name='', traj=None, Sim=None, configs=None, uct_tree_list=None, keep_uct_tree=False):
    ...
    mkdir("Results")
    if not configs:
        from config import _configs as configs
    ...
    configs["min_vout"] = -500
    configs["max_vout"] = 500
    ...
    for traj in traj_list:
        ...
        if configs["algorithm"] == "UCF" or configs["algorithm"] == "UCT":
            if not configs["leaf_parallel"]:
                result, anal_results, simu_results, save_tops, filted_save_tops = \
                    serial_UCF_test(trajectory=traj,
                                    test_number=configs[
                                        'test_number'],
                ...
                result, anal_results, simu_results, save_tops, filted_save_tops = \
                    leaf_parallel_UCF_test(trajectory=traj,
                                           test_number=configs[
                                               'test_number'],
                                           result_folder=result_folder,
                                           configs=configs,
            ...
            if Sim is None:  # just for uct with simulator or analytics
                ...
                filted_save_output_tops[traj] = filted_save_tops
        elif configs["algorithm"] == "Collect":
            result, anal_results, simu_results, save_tops, filted_save_tops = collect_data_with_UCT(trajectory=traj,
                                                                                                    test_number=configs[
                                                                                                        'test_number'],
                                                                                                    result_folder=result_folder,
                                                                                                    configs=configs,
    ...
    simu_top_file_name = "Results/BF-" + str(configs["target_vout"]) + "-save-filted-topks-" + date_str + "-" + str(
        os.getpid()) + ".json"
    with open(simu_top_file_name, 'w') as f:
        json.dump(filted_save_output_tops, f)
