...
def generate_one_data_json(name, directory_path, graph, comp2port_mapping, port2comp_mapping, idx_2_port, port_2_idx, parent,
    ...
    if nx.is_connected(G) and not has_short_cut:
        ...
        prohibit_path = ['VIN - L - GND', 'VIN - L - VOUT', 'VOUT - L - GND',
                         ...
                         'VIN - Sa - Sa - GND', 'VIN - Sb - Sb - GND', 'VOUT - Sa - Sa - GND', 'VOUT - Sb - Sb - GND',
                         'VIN - Sa - Sa - Sa - GND', 'VIN - Sb - Sb - Sb - GND',
                         'VOUT - Sa - Sa - Sa - GND', 'VOUT - Sb - Sb - Sb - GND']

        path = find_paths_from_edges(list_of_node, list_of_edge)
        #            print(path)
        if not check_topo_path(path, prohibit_path):
            warnings.warn('still has prohibit path', UserWarning)
            print("errrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrror!")
            return "Has prohibit path"
        elif not check_switch(path):
            warnings.warn('Not has switch', UserWarning)
            return "Not has switch"
        elif check_redundant_loop(list_of_node,list_of_edge):
            warnings.warn('has redundant loop', UserWarning)
            return "Has redundant loop"

        # if not check_topo_path(path, prohibit_path) or not check_switch(path):
    ...
    with open(directory_path + '/' + name + '-data.json', 'w') as outfile:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n_components', type=int, default=3, help='specify the number of component')
    parser.add_argument('-n_topology', type=int, default=2000,
