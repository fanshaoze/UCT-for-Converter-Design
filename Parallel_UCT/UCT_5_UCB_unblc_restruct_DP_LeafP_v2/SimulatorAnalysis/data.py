...
def generate_key_data(directory_path, name, target_folder, data_file_content=None):
    if not data_file_content:
        json_file = json.load(open(directory_path + '/' + name + '-data.json'))
    else:
        json_file = copy.deepcopy(data_file_content)
    ...
    for fn in json_file:
        ...
        list_of_edge_list = []
        for edge in tmp[fn]['list_of_edge']:
            list_of_edge_list.append(list(edge))
        tmp[fn]['list_of_edge'] = list_of_edge_list
