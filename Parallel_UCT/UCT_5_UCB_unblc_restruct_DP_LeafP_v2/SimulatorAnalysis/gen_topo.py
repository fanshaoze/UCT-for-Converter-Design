...
from SimulatorAnalysis.utils_new.graphUtils import indexed_graph_to_adjacency_matrix, adj_matrix_to_graph, \
...
class TopoGraph(object):
    ...
    def encode_path_as_string(self, path, with_count=False):
        """
    ...
    def find_end_points_paths_as_str(self, with_count=False):
        paths = self.find_end_points_paths()
        paths_str = [self.encode_path_as_string(path, with_count) for path in paths]
...
def find_paths_from_edges(node_list, edge_list, with_count=False):
    return TopoGraph(node_list=node_list, edge_list=edge_list).find_end_points_paths_as_str(with_count)
...
def find_same_designs(node_list, edge_list, net_list):
    ...
    num_list = [num_C, num_L, num_Sa, num_Sb]
    name_list = ['C', 'L', 'Sa', 'Sb']

    num_comp = len(net_list)
    count_max = max(num_C, 1) * max(num_L, 1) * max(num_Sa, 1) * max(num_Sb, 1)
    component_list = []
    ...
    comp_all = [C_all, L_all, Sa_all, Sb_all]
...
def key_circuit_using_lists(edge_list, node_list, net_list):
    path = find_paths_from_edges(node_list, edge_list)
    node_dic = {}
    node_name = {}
    net_list_dic = {}
...
def exp_subs(a_x, a_y, a_A, a_B, a_C, a_D, device, value):
    a_xp = a_x.replace(device, value)
    a_yp = a_y.replace(device, value)
    a_Ap = a_A.replace(device, value)
    a_Bp = a_B.replace(device, value)
    a_Cp = a_C.replace(device, value)
    a_Dp = a_D.replace(device, value)
