...
class TopoGenSimulator(uct.Simulator):
    ...
    def update_action_set(self):
        ...
        # finish adding component and edge, but not terminal, means no sweep
    ...
    def get_sweep_reward_with_para(self):
        ...
        # uncomment the following lines to enable filtering using GNN
        # self.filter_sim.current = self.current
        # self.reward, self.effi, self.vout, self.current.parameters = 0, 0, -500, [0.5, 10, 100]
        # reward = self.filter_sim.get_reward()
        # if reward < self.configs_['reward_threshold']:
        #     reward = 0
        #     if tmp_max_reward < reward:
        #         tmp_max_reward = reward
        #         tmp_max_para = [0.5, 10, 100]
        # else:
    ...
    def get_reward_using_anal(self):
        ...
        if not self.is_terminal():
            # init as invalid
            self.reward, self.effi, self.vout, self.current.parameters = 0, 0, -500, []
        ...
        if self.graph_2_reward.__contains__(key + '$' + str(self.current.parameters)):
            ...
            if self.configs_['skip_sim'] and \
                ...
                if (key + '$' + C_L_list_str in self.key_expression) and \
                    ...
                    # self.graph_2_reward[key + '$' + str(tmp_para)] = [tmp_para, tmp_effi, tmp_vout]
                    # self.update_key_sta(key, tmp_reward, True)
                # para_result = find_one_analytics_result(current=self.current,
                #                                         key=key, key_expression=self.key_expression,
