...
class UCTPlanner(object):
    ...
    def set_and_plan(self, total_traj, reward_hash, conn):
        while True:
            time.sleep(0.5)
            action = conn.recv()
            new_root = conn.recv()
            ...
            if action == -2:
                ...
                self.set_root_node(_state=new_root.state_, _act_vect=new_root.act_vect_,
                                   _next_candidate_components=new_root.next_candidate_components_,
                                   _current_candidate_components=new_root.current_candidate_components_,
                                   _weights=new_root.weights_, _reward=new_root.reward_,
                                   _is_terminal=new_root.is_terminal_)
            else:
    ...
    def plan(self, total_traj, get_viz=False):
        """
        The MCTS process
    ...
    def update_root_node(self, act, new_state):
        ...
        print('total action num is:', len(self.root_.act_vect_), 'update: ', len(self.root_.node_vect_),
              'by selecting: ', act_ptr)
        if act_ptr >= len(self.root_.node_vect_):
            # we meets a situation that the main process selects a root that has not been expends on this sub-tree
            while len(self.root_.node_vect_) <= act_ptr:
                act_ID = self.root_.add_action_node()
            action_node = self.root_.node_vect_[act_ptr]
            action_to_take = self.root_.act_vect_[act_ptr]
            depth = 1
            current = self.root_
            self.sim_.set_state(current.next_candidate_components_, current.current_candidate_components_,
                                current.state_)

            r = self.sim_.act(action_to_take)
            next_node = current.node_vect_[act_ptr].add_state_node(self.sim_.get_state(),
                                                                   ...
                                                                   self.sim_.get_weights(),
                                                                   r, self.sim_.is_terminal())
            current.node_vect_[act_ptr].set_visit_weight(self.sim_.get_action_weights(len(current.act_vect_),
                                                                                      current.state_.component_pool,
                                                                                      current.next_candidate_components_,
                                                                                      current.current_candidate_components_,
                                                                                      next_node.state_.component_pool,
