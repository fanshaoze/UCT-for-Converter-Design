...
class UCTPlanner(object):
    ...
    def plan(self, step_traj, get_viz=False, step_count=0, thread_id=9, return_dict={}):
                    ...
                    if current == self.root_:
                        ...
                        for i in range(0, size):
                            ...
                            if current.node_vect_[i].rave_num_visits_ == 0:
                                ...
                                #          " rave_sqrt:" + str(math.sqrt(rave_det / float(current.node_vect_[i].rave_num_visits_))) +
                                #          " rave_Qv':" + str(
                                #     float(current.node_vect_[i].rave_num_visits_) * current.node_vect_[i].rave_avg_return_) +
        ...
        #     else:
        #         fo.write("not_zero:" + str(not_zero) + " percentage: " + str(not_zero/len(update_sim_lists[i])) + "\n")
        #     fo.write("\n")
        #
        # fo.close()
        if get_viz:
    ...
    def get_action_rave(self):
        return self.root_.act_vect_[self.get_greedy_branch_index_rave()]
    ...
    def get_greedy_branch_index_rave(self):
        assert self.root_ is not None
        maximizer = []
        size = len(self.root_.node_vect_)
        self.beta_ = math.sqrt(self.rave_k_ / (3 * self.root_.num_visits_ + self.rave_k_))
        for i in range(0, size):
            maximizer.append(self.beta_ * self.root_.node_vect_[i].rave_avg_return_ +
                             (1 - self.beta_) * self.root_.node_vect_[i].avg_return_)
        return maximizer.index(max(maximizer))
    ...
    def get_UCT_root_index_rave(self, node):
        det = math.log(float(node.num_visits_))
        if node.rave_num_visits_ == 0:
            rave_det = 0
        else:
            rave_det = math.log(float(node.rave_num_visits_))

        maximizer = []  # maximizer.clear()
        size = len(node.node_vect_)
        self.beta_ = math.sqrt(self.rave_k_ / (3 * node.num_visits_ + self.rave_k_))
        for i in range(0, size):
            val = node.node_vect_[i].avg_return_
            val += self.ucb_scalar_ * math.sqrt(det / float(node.node_vect_[i].num_visits_))

            if node.node_vect_[i].rave_num_visits_ == 0:
                rave_val = 0
            else:
                rave_val = node.node_vect_[i].rave_avg_return_
                rave_val += self.rave_scalar_ * math.sqrt(rave_det / float(node.node_vect_[i].rave_num_visits_))
            maximizer.append(self.beta_ * rave_val + (1 - self.beta_) * val)
        return maximizer.index(max(maximizer))
    ...
    def get_UCT_branch_index_rave(self, node):
        """
        Use the UCB to select the next branch
        """
        det = math.log(float(node.num_visits_))
        ...
        for i in range(0, size):
            ...
            if node.node_vect_[i].rave_num_visits_ == 0:
                ...
                rave_val += self.rave_scalar_ * math.sqrt(rave_det / float(node.node_vect_[i].rave_num_visits_))
                if node.rave_num_visits_ == 0:
                    warnings.warn("rave_det is 0 but act's rave_num_visits_ is not", UserWarning)
    ...
    def update_values_rave(self, node, mc_return, sampling_action_list):
        total_return = mc_return
        if node.num_visits_ == 0:
            node.first_MC_ = total_return
        node.num_visits_ += 1
        ...
        while node.parent_act_ is not None:
            ...
            # parent_act.rave_num_visits_ += 1
            # parent_act.rave_avg_return_ += (total_return - parent_act.rave_avg_return_) \
            #                                / parent_act.rave_num_visits_  # / for divide
            node.num_visits_ += 1
            # node.rave_num_visits_ += 1

            for sibling_index in range(len(node.node_vect_)):
                if node.node_vect_[sibling_index] == parent_act:
                    continue
                for sampled_action in sampling_action_list:
                    if node.act_vect_[sibling_index].equivalent(sampled_action):
                        node.rave_num_visits_ += 1
                        if node.node_vect_[sibling_index] is None:
                            print("not possible")
                        node.node_vect_[sibling_index].rave_num_visits_ += 1
                        node.node_vect_[sibling_index].rave_avg_return_ += \
                            (total_return - node.node_vect_[sibling_index].rave_avg_return_) \
                            / node.node_vect_[sibling_index].rave_num_visits_  # / for divide
                        break
    ...
    def MC_sampling_depth_rave(self, node, depth):
        """
        playing out process, with a limited depth
        """
        max_reward_flag = 0
        ...
        for i in range(0, depth):
            ...
            act_ID = int(random.random() * len(actions))
            r = self.sim_.act(actions[act_ID])
            sampling_action_list.append(actions[act_ID])
            # if r > mc_return:
            #     mc_return = discnt * self.modify_reward(r)
            mc_return += discount * self.modify_reward(r)
            if not final_return or (final_return < mc_return):
                max_reward_flag = len(sampling_action_list)
                final_return = mc_return
            discount *= self.gamma_
        self.sim_.get_state()
        if not final_return:
            final_return = 0
        # return mc_return
        return final_return, sampling_action_list[:max_reward_flag]
    ...
    def MC_sampling_terminal_rave(self, node):
        """
        playing out process, until reach a terminal state
        """
        max_reward_flag = 0
        ...
        while not self.sim_.is_terminal():
            actions = self.sim_.get_actions()
            act_ID = int(random.random() * len(actions))
            sampling_action_list.append(copy.deepcopy(actions[act_ID]))
            r = self.sim_.act(actions[act_ID])
            reward_list.append(r)
            # print("diff test:----------------", origin_reward, sum(reward_list), self.sim_.get_reward())
            # if origin_reward + sum(reward_list) != self.sim_.get_reward():
    ...
    def update_root_node(self, act, new_state):
        ...
        if action_node.state_vect_ is None:
            ...
            if -1 == self.max_depth_:
                ...
                # mc_return = self.MC_sampling_depth(next_node, self.max_depth_ - depth)
                mc_return, sampling_action_list = self.MC_sampling_depth_rave(next_node, self.max_depth_ - depth)
            current = next_node
            self.update_values(current, mc_return)
            # self.update_values_rave(current, mc_return)
            self.root_ = current
