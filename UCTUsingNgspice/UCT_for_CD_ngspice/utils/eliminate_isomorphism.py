...
from utils.util import get_args, get_sim_configs, mkdir, read_reward_hash, get_topology_from_hash, read_reward_hash_list
...
def init_candidate_parameters():
    ...
    candidate_parameters["capacitor"] = [5 / 200]
    # candidate_parameters["inductor"] = [1 / 200, 10 / 200, 50 / 200, 150 / 200]
    candidate_parameters["inductor"] = [50 / 200, 150 / 200]
    candidate_parameters["FETA"] = [10000 / 100000]
    candidate_parameters["FETB"] = [25000 / 100000]
...
def instance_to_isom_str(current):
    """
...
def try_cap_para(target_ins, candidate_cap_parameters, configs):
    ...
    for cap_para in candidate_cap_parameters:
        ...
        if not effi[0].__contains__('output_voltage'):
            ...
            reward = calculate_reward(effi[0])
