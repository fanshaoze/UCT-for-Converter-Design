...
def serial_UCF_test(depth_list, trajectory, configs, date_str):
    ...
    for max_depth in depth_list:
        for num_runs in trajectory:
            ...
            for v in init_nodes:
                action = TopoPlanner.TopoGenAction('node', v)
                sim.act(action)
            edges = []
            ...
            uct_tree.set_root_node(sim.get_state(), sim.get_actions(), sim.get_weights(), r, sim.is_terminal())
            for n in range(configs["tree_num"]):
                uct_tree_list[n].set_root_node(sim.get_state(), sim.get_actions(),sim.get_weights(), r, sim.is_terminal())
            ...
            # True: for using fixed trajectory on every step(not decreasing trajectory)
            ...
            for _ in range(1):
                ...
                step_traj = steps_traj[sim.current.step - len(init_nodes) -
                                       (configs['num_component'] - len(['GND', 'VIN', 'VOUT']) - len(init_nodes))]
                ...
                for n in range(configs["tree_num"]):
                    ...
                    #     uct_tree_list[n].plan(step_traj, False, sim.current.step - (len(edges) + len(init_nodes)))

                    tree_size_tmp, tree_tmp, depth = \
                        uct_tree_list[n].only_explor_plan(step_traj, False, sim.current.step - (len(edges) + len(init_nodes)))
            ...
            result = result + "#Efficiency:" + str(effis[0]['efficiency']) + "#FinalRewards:" + str(
                ...
                total_query) + "#TreeSize:" + str(tree_size) + "#HashSize:" + str(len(uct_simulators[0].graph_2_reward))
