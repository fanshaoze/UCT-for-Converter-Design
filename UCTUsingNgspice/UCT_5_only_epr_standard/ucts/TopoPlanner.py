...
class TopoGenSimulator(uct.Simulator):
    def __init__(self, _configs, _approved_path_freq, _component_condition_prob, _key_expression={}, _key_sim_effi={}, _fix_para_set=None,
    ...
    def get_reward(self):
        ...
        if self.graph_2_reward.__contains__(key):
            ...
            self.topk.append([self.current.duplicate(), self.reward, self.current.parameters])
            if len(self.topk) > 10:
        ...
        for i in range(len(self.topk)):
            ...
            if current.graph_is_valid():
                results_tmp = get_analytics_file(self.key_expression, current.graph,
                                                 ...
                                                 [current.topk[i][5]], self.configs_['min_vout'])
            else:
                results_tmp = None
            effi_info = self.simulate_one_analytics_result(results_tmp)
            # [key, state, anal_reward, anal_effi, anal_vout, anal_para, sim_reward, sim_effi, sim_vout]
            self.topk[i][7] = effi_info['efficiency']
            self.topk[i][8] = effi_info['Vout']
            effi = {'efficiency': self.topk[i][7], 'output_voltage': self.topk[i][8]}
            self.topk[i][6] = calculate_reward(effi, self.configs_['target_vout'],
                                               self.configs_['min_vout'],
                                               self.configs_['max_vout'])
