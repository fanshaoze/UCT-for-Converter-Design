...
def get_sim_configs(configs):
    sim_configs = {"sys_os": configs["sys_os"], "output": configs["output"], "freq": configs["freq"],
                   ...
                   "min_vout": configs["min_vout"], 'topk_size': configs["topk_size"],
                   'DP_component_basic_weight': configs['DP_component_basic_weight'],
                   'DP_path_basic_weight': configs['DP_path_basic_weight']}
    return sim_configs
...
def remove_tmp_files():
    ...
    os.system("rm PCC-*.simu")
    os.system("rm SimulatorAnalysis/database/PCC*.json")
