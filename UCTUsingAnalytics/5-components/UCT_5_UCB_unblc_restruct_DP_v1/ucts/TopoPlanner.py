...
class TopoGenSimulator(uct.Simulator):
    ...
    def set_state(self, _next_candidate_components, _current_candidate_components, state):
    ...
    def get_effi_info(self):
        """
        just a version Get_reward that return the Effi and Vout
        """
        if not self.current.graph_is_valid():
            self.current.parameters = 'None'
            self.reward = 0
    ...
    def current_sim_reward(self):
        """
        get simulation information about current topo
        """
        if self.current.graph_is_valid():
