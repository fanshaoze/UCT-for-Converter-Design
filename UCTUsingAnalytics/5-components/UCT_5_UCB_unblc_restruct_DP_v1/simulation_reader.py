...
import datetime
from utils.util import mkdir, get_args, get_sim_configs, read_approve_path, read_joint_component_prob
...
def main(name, try_target_vout, result_folder, trajs, rounds):
    ...
    for trajectory in trajs:
        ...
        for test_idx in range(rounds):
            ...
            results_tmp = sim.generate_topk_simulation_base_dict()
            ...
            for k, v in topk_json.items():
                ...
                results_tmp = v[0]
                effi_info = sim.simulate_one_analytics_result(results_tmp)
                max_para_reward, max_para_effi_info, max_para = sim.find_simu_max_reward_para(effi_info)
