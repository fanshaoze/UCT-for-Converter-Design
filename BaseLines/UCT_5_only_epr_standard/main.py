...
def main(name):
    ...
    anal_output_results = [[] for i in range(test_number + 1)]
    simu_output_results = [[] for i in range(test_number + 1)]
    ...
    for try_target_vout in target_vout_list:
        for traj_list in traj_lists:
            ...
            if configs["algorithm"] == "UCF" or configs["algorithm"] == "UCT":
                ...
                anal_results, simu_results = genetic_search(traj_list, configs, date_str)
            ...
            #     hash_read_test(depth_list, traj_list, configs, date_str)
                ...
                anal_results, simu_results = brute_force(depth_list, traj_list, configs, date_str)
                for i in range(test_number + 1):
                    anal_output_results[i].extend(anal_results[i])
                    simu_output_results[i].extend(simu_results[i])
        anal_out_file_name = "Results/"+configs["algorithm"]+"-" + str(try_target_vout) + "-anal-" + date_str + "-" + str(os.getpid()) + ".csv"
        with open(anal_out_file_name, 'w') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerows(anal_output_results)
        f.close()
        simu_out_file_name = "Results/"+configs["algorithm"]+"-" + str(try_target_vout) + "-simu-" + date_str + "-" + str(os.getpid()) + ".csv"
